...
public class SetSMS extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		save_btn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View arg0) {
				if(isEdit){//如果是对一条已有的短信进行编辑之后保存时，调用update
					updateSMS(sms);
				}
				else{//如果是新加一条短信时，则将该条短信加入到数据库
					addSMS(sms);
				}
			}
		});
	...
	protected void addSMS(SMS sms) {
		...
		if(sms.phone.isEmpty()||sms.message.isEmpty()){
			Toast.makeText(getApplicationContext(), "号码或者信息内容不能为空",
		...
		if(isTimeLegal(sms.year,sms.month,sms.date,sms.hour,sms.minutes)){
			...
			Toast.makeText(getApplicationContext(), "短信发送时间必须在当前时间之后",
				     Toast.LENGTH_SHORT).show();
		}
	}
